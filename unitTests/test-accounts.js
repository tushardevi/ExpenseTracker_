...
test('REGISTER : register and log in with a valid member account', async test => {
  ...
  const data = {firstName:'first', lastName:'last', username:'user0', email:'email@hotmail.com', password: 'password'}
...
test('REGISTER : register a duplicate username', async test => {
  ...
  const data = {firstName:'first', lastName:'last', username:'user0', email:'email@hotmail.com', password: 'password'}
  const data2 = {firstName:'first', lastName:'last', username:'user0', email:'email2@hotmail.com', password: 'password'}
	try {
		await account.register(data)
...
test('REGISTER : error if duplicate email', async test => {
	test.plan(1)
	const account = await new Accounts()
  const data = {firstName:'first', lastName:'last', username:'user00', email:'email@hotmail.com', password: 'password'}
  const data2 = {firstName:'first', lastName:'last', username:'user0', email:'email@hotmail.com', password: 'password'}
	try {
		await account.register(data)
		await account.register(data2)
		...
		test.is(err.message,'email address "email@hotmail.com" is already in use', 'incorrect error message')
	} finally {
		account.close()
	}
})
...
test('REGISTER : error if blank first name', async test => {
	test.plan(1)
	const account = await new Accounts()
  const data = {firstName:'', lastName:'last', username:'', email:'email@hotmail.com', password: 'password'}
	try {
		await account.register(data)
...
test('REGISTER : error if blank last name', async test => {
	test.plan(1)
	const account = await new Accounts()
  const data = {firstName:'first', lastName:'', username:'', email:'email@hotmail.com', password: 'password'}
	try {
		await account.register(data)
...
test('REGISTER : error if blank username', async test => {
	test.plan(1)
	const account = await new Accounts()
  const data = {firstName:'first', lastName:'last', username:'', email:'email@hotmail.com', password: 'password'}
	try {
		await account.register(data)
...
test('REGISTER : error if blank password', async test => {
	test.plan(1)
	const account = await new Accounts()
  const data = {firstName:'first', lastName:'last', username:'user0', email:'email@hotmail.com', password: ''}
	try {
		await account.register(data)
...
test('REGISTER : error if blank email', async test => {
	test.plan(1)
	const account = await new Accounts()
  const data = {firstName:'first', lastName:'last', username:'user0', email:'', password: 'password'}
	try {
		await account.register(data)
...
test('GET ALL USERS  : check if all users are correctly retrieved from the db', async test => {
	...
	const account = await new Accounts()
  //create 2 accounts
  const user1 = {firstName:'first', lastName:'last', username:'user0', email:'email@hotmail.com', password: 'password'}
  const user2 = {firstName:'first2', lastName:'last2', username:'user00', email:'email2@hotmail.com', password: 'password'}
